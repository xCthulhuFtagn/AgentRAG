...
- Each project holds uploaded files (`.pdf/.docx/.pptx/.txt/.md`). **"Edit files"** opens a staging session: add / rename / delete as many as you want — nothing touches disk yet. Each file's pencil icon also opens a language picker (searchable checkboxes, one or more languages) to manually set its OCR/description language instead of auto-detecting — useful for a mixed-language corpus or when detection guesses wrong; leaving none checked keeps auto-detect. **Done & update index** applies everything at once and indexes **only the delta**: new/replaced/renamed files and files whose language changed are (re)indexed, deleted files' tables are dropped, untouched files keep their existing tables; **Cancel** discards.
- A project's ⋮ menu carries two settings dialogs. **Indexing settings** holds the index-time hyperparameters — chunk size, chunk overlap, LLM file descriptions on/off, description excerpt size; applying any change rebuilds the whole project index (old chunks were cut with the old settings), so its button reads **Apply — full reindex**. **RAG process settings** holds the search-time knobs — *k blocks* (nearest chunks per search), stitch padding, merge gap, LLM reranking, and the iteration budget (max search-and-judge loop iterations) — which apply from the next search with no reindex. Global `.env` values are the defaults; saved per project in its `meta.json`. Each dialog's red **Apply** appears only once a value differs from the saved one (and saving one dialog never resets the other's overrides); closing without changes does nothing.
...
- **Per-project overrides (web)** — `CHUNK_SIZE`, `CHUNK_OVERLAP`, `DESCRIPTIONS_ENABLED`, `DESCRIBE_MAX_CHARS` (the *Indexing settings* dialog) plus the search-time `SEARCH_TOP_K`, `EXPAND_PADDING`, `BRIDGE_GAP`, the reranking toggles and `MAX_ITERATIONS` (the *RAG process settings* dialog) can be overridden per project; the `.env` values above are the defaults for projects that never changed them. Applying an index-time override triggers that project's full reindex; search-time overrides apply from the next search (threaded through graph state into `search_fanout` / `gather_neighbors`).

> Changes to chunking/embedding settings require a **reindex** to take effect (web: the *Indexing settings* dialog's **Apply** does it; CLI: re-run the indexer). Search/stitching settings (`SEARCH_TOP_K`, `EXPAND_PADDING`, `BRIDGE_GAP`, `MAX_EXPANDED`; web: the *RAG process settings* dialog) apply immediately on the next query — no reindex needed.
