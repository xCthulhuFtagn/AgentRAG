...
- «прирост по теме: X₁/N₁ → X₂/N₂ → …» — динамика того, сколько НОВЫХ чанков каждого поиска LLM оценил как релевантные вопросу. Спад (например 12/15 → 3/13 → 1/3) означает: коллекция отдаёт всё менее релевантный материал. Это ИСЧЕРПАННОСТЬ — не путай с продуктивностью. Выбирать «есть_неиспробованный_угол» при спаде прироста НЕЛЬЗЯ, даже если формально +N > 0.
