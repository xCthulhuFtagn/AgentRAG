...
class SufficientContextResult(BaseModel):
    ...
    verdict: JudgeVerdict = Field(
        description=(
            ...
            "• «исчерпано_есть_упоминания» — все правдоподобно-релевантные коллекции обысканы, новые поиски не приносят нового ПО ТЕМЕ (последний поиск «+0 новых», или чанки не о том, или «прирост по теме» устойчиво падает — даже если +N ещё не ноль), а draft_answer собрал всё найденное, пусть и скудное («в источниках об этом есть только …») — это полноценный честный ответ;\n"
            "• «есть_необысканная_коллекция» — в описи есть правдоподобно-релевантная коллекция, которой НЕТ в статистике поисков;\n"
            "• «есть_неиспробованный_угол» — релевантная коллекция ещё отдаёт НОВЫЕ по теме чанки (её последний поиск НЕ «+0», И «прирост по теме» не на спаде — новые чанки действительно о том, что спрошено) и есть конкретная неиспробованная формулировка; при «+0 новых» или при падающем приросте эту ситуацию для такой коллекции выбирать НЕЛЬЗЯ;\n"
