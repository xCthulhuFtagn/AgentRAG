...
# so the LLM's httpx async client is reused across tests; with the default
...
def _skip_on_llm_failure(state: dict) -> None:
    """Skip when the run aborted on a transient LLM failure, not a verdict.
    ...
    if err:
        pytest.skip(f"transient LLM failure, not a logic outcome: {err[:160]}")
