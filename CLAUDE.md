...
- **Mechanical search statistics (code computes, the model reads)** — a weak model can't reconstruct "what has been searched" from collection tags scattered across the retrieved chunks (it hallucinated "not searched yet" for a collection searched on iteration 1). `collection_search_stats` (`agents/common.py`) walks the accumulated `search_results` and the nodes inject pre-formatted ground truth: the **judge** gets the searched set + last-search novelty delta (*«обыскана 3 раза, последний поиск дал +0 новых чанков»* — `+0` is the diminishing-returns exhaustion detector that kills the "let's search there again" loop), the **Planner** gets per-collection coverage (*«извлечено 28/210 чанков (13%)»*, totals via `count_chunks` in `vectordb/tools.py`) framed as a routing signal, never a verdict criterion. The judge deliberately gets deltas, not percentages — a weak model reads "13% seen" as "87% unexplored → keep searching". For this record to be complete, `search_fanout` keeps **every** executed search in `search_results`, empty ones included (an empty search is the strongest exhaustion signal); renderers (judge/synthesis/give_up) skip chunkless entries.
...
**Re-routing on iteration.** The loop re-enters at the **Planner**: with `feedback` + `iteration_count > 0` set, `planner` uses `PLANNER_ITERATION_PROMPT` to re-route to the collection(s) most likely to hold the missing piece — matching the gap (and its alternative phrasings) against collection descriptions and the coverage statistics, preferring unsearched collections. If the Planner finds no relevant route — initial turn or iteration — it goes straight to `give_up` (pure RAG: no broad fallback). `query_rewriter` therefore has a single mode: rewrite the Planner's routes, one search task per route. Max iterations: 3.
